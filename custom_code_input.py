    ...
    def keyboard_on_key_up(self, window, keycode):
        ...
        if not self.blocker and keycode[1] == "s":
            ...
            if app.focused_md_file is None:
                ...
                return
            if self.is_current_lexer_markdown:
                app.save_to_md_file(
                    app.focused_md_file.timestamp, app.focused_md_file.path, text
                )
            else:
                parsed_text = yaml.load(text, Loader=yaml.FullLoader)
                app.save_to_yaml_file(
                    app.focused_md_file.timestamp, app.focused_md_file.path, parsed_text
                )
